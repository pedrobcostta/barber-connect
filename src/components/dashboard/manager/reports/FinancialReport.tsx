...
export function FinancialReport({ dateRange }: ReportProps) {
  ...
  const fetchReportData = async (from: Date, to: Date) => {
    ...
    if (error) {
      setError('Falha ao carregar o relatório financeiro.');
  ...
  if (!data || !data.kpis) return <p>Nenhum dado encontrado para este período.</p>;

  return (
    <div className="space-y-6">
      <div className="grid gap-4 md:grid-cols-3">
        <KpiCard title="Faturamento Bruto" value={formatCurrency(data.kpis.total_revenue)} icon={DollarSign} />
        <KpiCard title="Custos Totais" value={formatCurrency(data.kpis.total_costs)} icon={TrendingDown} />
        <KpiCard title="Lucro Líquido" value={formatCurrency(data.kpis.net_profit)} icon={PiggyBank} />
      </div>
      <Card>
        <CardHeader><CardTitle>Faturamento vs. Custos</CardTitle></CardHeader>
        <CardContent>
